...
async function logToSupabase(logData) {
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_KEY) {
    console.warn('[Automation] Supabase not configured, skipping log write');
    return;
  }
...
function getOpenAIClient() {
  ...
  if (!openai) {
    try {
      ...
      console.warn('[Automation] Failed to initialize OpenAI client:', error.message);
      return null;
    }
  }
...
async function generateEmbedding(text) {
  try {
    ...
    if (!openaiClient) {
      console.warn('[Automation] OpenAI API key not configured, cannot generate embedding');
      return null;
    }
...
async function generateMissingEmbeddings() {
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_KEY) {
    console.warn('[Automation] Supabase not configured, cannot generate embeddings');
    return {
      processed: 0,
      errors: 0,
      articlesProcessed: 0,
