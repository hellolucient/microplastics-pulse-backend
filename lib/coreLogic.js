...
// --- Initialize Clients ---
// Lazy initialization for Supabase - only create client when needed and if credentials are available
let supabase = null;
...
function getSupabaseClient() {
  ...
  // Initialize client only if not already created
  if (!supabase) {
    try {
      supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
    } catch (error) {
      console.warn('[CoreLogic] Failed to initialize Supabase client:', error.message);
      return null;
    }
  }
...
// OpenAI client - initialize lazily as well
let openai = null;
...
function getOpenAIClient() {
  ...
  if (!openai) {
    try {
      ...
      console.warn('[CoreLogic] Failed to initialize OpenAI client:', error.message);
      return null;
    }
  }
...
const IMAGE_GEN_QUALITY = 'medium';
...
async function summarizeText(title, snippet) {
    if (!title || !snippet) {
        console.warn('[summarizeText] Missing required fields:', { hasTitle: !!title, hasSnippet: !!snippet });
        return null;
    }
    
    const openaiClient = getOpenAIClient();
    if (!openaiClient) {
        console.warn('[summarizeText] OpenAI API key not configured');
        return null;
    }
...
function determineCulturalContext(title, articleUrl) {
    ...
    // Comprehensive nationality detection
    const nationalityPatterns = [
        // Asian countries
        { keywords: ['japan', 'japanese', 'tokyo'], nationality: 'Japanese', domains: ['.jp'] },
        { keywords: ['china', 'chinese', 'beijing', 'shanghai'], nationality: 'Chinese', domains: ['.cn'] },
        { keywords: ['korea', 'korean', 'seoul'], nationality: 'Korean', domains: ['.kr'] },
        { keywords: ['india', 'indian', 'mumbai', 'delhi'], nationality: 'Indian', domains: ['.in'] },
        { keywords: ['singapore', 'singaporean'], nationality: 'Singaporean', domains: ['.sg'] },
        { keywords: ['thailand', 'thai', 'bangkok'], nationality: 'Thai', domains: ['.th'] },
        { keywords: ['vietnam', 'vietnamese', 'ho chi minh'], nationality: 'Vietnamese', domains: ['.vn'] },
        { keywords: ['philippines', 'filipino', 'manila'], nationality: 'Filipino', domains: ['.ph'] },
        { keywords: ['indonesia', 'indonesian', 'jakarta'], nationality: 'Indonesian', domains: ['.id'] },
        { keywords: ['malaysia', 'malaysian', 'kuala lumpur'], nationality: 'Malaysian', domains: ['.my'] },
        
        // European countries
        { keywords: ['germany', 'german', 'berlin', 'munich'], nationality: 'German', domains: ['.de'] },
        { keywords: ['france', 'french', 'paris', 'lyon'], nationality: 'French', domains: ['.fr'] },
        { keywords: ['uk', 'britain', 'british', 'london', 'england'], nationality: 'British', domains: ['.uk', '.co.uk'] },
        { keywords: ['italy', 'italian', 'rome', 'milan'], nationality: 'Italian', domains: ['.it'] },
        { keywords: ['spain', 'spanish', 'madrid', 'barcelona'], nationality: 'Spanish', domains: ['.es'] },
        { keywords: ['netherlands', 'dutch', 'amsterdam'], nationality: 'Dutch', domains: ['.nl'] },
        { keywords: ['sweden', 'swedish', 'stockholm'], nationality: 'Swedish', domains: ['.se'] },
        { keywords: ['norway', 'norwegian', 'oslo'], nationality: 'Norwegian', domains: ['.no'] },
        { keywords: ['denmark', 'danish', 'copenhagen'], nationality: 'Danish', domains: ['.dk'] },
        { keywords: ['finland', 'finnish', 'helsinki'], nationality: 'Finnish', domains: ['.fi'] },
        { keywords: ['poland', 'polish', 'warsaw'], nationality: 'Polish', domains: ['.pl'] },
        { keywords: ['russia', 'russian', 'moscow'], nationality: 'Russian', domains: ['.ru'] },
        
        // American countries
        { keywords: ['usa', 'united states', 'america', 'american', 'california', 'new york'], nationality: 'American', domains: ['.us'] },
        { keywords: ['canada', 'canadian', 'toronto', 'vancouver'], nationality: 'Canadian', domains: ['.ca'] },
        { keywords: ['mexico', 'mexican', 'mexico city'], nationality: 'Mexican', domains: ['.mx'] },
        { keywords: ['brazil', 'brazilian', 'sao paulo', 'rio de janeiro'], nationality: 'Brazilian', domains: ['.br'] },
        { keywords: ['argentina', 'argentinian', 'buenos aires'], nationality: 'Argentinian', domains: ['.ar'] },
        { keywords: ['chile', 'chilean', 'santiago'], nationality: 'Chilean', domains: ['.cl'] },
        
        // African countries
        { keywords: ['south africa', 'south african', 'cape town', 'johannesburg'], nationality: 'South African', domains: ['.za'] },
        { keywords: ['nigeria', 'nigerian', 'lagos'], nationality: 'Nigerian', domains: ['.ng'] },
        { keywords: ['egypt', 'egyptian', 'cairo'], nationality: 'Egyptian', domains: ['.eg'] },
        { keywords: ['kenya', 'kenyan', 'nairobi'], nationality: 'Kenyan', domains: ['.ke'] },
        
        // Middle Eastern countries
        { keywords: ['israel', 'israeli', 'tel aviv'], nationality: 'Israeli', domains: ['.il'] },
        { keywords: ['saudi arabia', 'saudi', 'riyadh'], nationality: 'Saudi', domains: ['.sa'] },
        { keywords: ['uae', 'united arab emirates', 'dubai'], nationality: 'Emirati', domains: ['.ae'] },
        { keywords: ['turkey', 'turkish', 'istanbul'], nationality: 'Turkish', domains: ['.tr'] },
        
        // Oceania
        { keywords: ['australia', 'australian', 'sydney', 'melbourne'], nationality: 'Australian', domains: ['.au'] },
        { keywords: ['new zealand', 'zealand', 'auckland'], nationality: 'New Zealander', domains: ['.nz'] }
    ];
    
    // Check for specific nationality matches
    for (const pattern of nationalityPatterns) {
        const hasKeyword = pattern.keywords.some(keyword => titleLower.includes(keyword));
        const hasDomain = pattern.domains.some(domain => urlLower.includes(domain));
        
        if (hasKeyword || hasDomain) {
            return {
                context: pattern.nationality,
                instruction: `For ${pattern.nationality} articles: Include ${pattern.nationality} people in professional settings, wearing appropriate business attire or lab coats`
            };
        }
    }
    
    // Check for general regional terms
    ...
    if (titleLower.includes('asia') || titleLower.includes('asian')) {
        return {
            context: 'Asian',
    ...
    if (titleLower.includes('africa') || titleLower.includes('african')) {
        return {
            context: 'African',
            instruction: 'For African articles: Include African people in professional settings, wearing appropriate business attire or lab coats'
        };
    }
...
async function generateAndStoreImage(title, articleUrl) {
    // Check for API key (not the client, which is lazily initialized later)
    if (!process.env.OPENAI_API_KEY || !process.env.BLOB_READ_WRITE_TOKEN || !title || !articleUrl) {
        console.error("generateAndStoreImage: Pre-requisites not met.", {
...
The image should communicate scientific research themes through visual symbolism only, without any textual elements whatsoever.`;
    ...
    try {
        ...
        if (!b64Json) {
            const noDataError = new Error('No image data returned from OpenAI');
        ...
        if (!imageBuffer.length) {
            const bufferError = new Error('Failed to decode image data from OpenAI');
            await logImageGenerationUsage(
                'openai',
                IMAGE_GEN_MODEL,
                'image_generation',
        ...
        const duration = Date.now() - startTime;
        const errorMessage = error?.error?.message || error?.message || 'Unknown image generation error';
        ...
        if (errorMessage.includes('safety system')) {
            ...
            console.error('Error in generateAndStoreImage:', errorMessage);
            await logImageGenerationUsage(
                'openai',
                IMAGE_GEN_MODEL,
                'image_generation',
...
async function processQueryAndSave(query) {
    ...
    if (!supabaseClient) {
        console.warn('[processQueryAndSave] Supabase not configured, cannot process query');
    ...
    while (true) {
        const { data: pageData, error: urlFetchError } = await supabaseClient
            .from('latest_news')
            .select('url')
    ...
    for (const article of articles) {
        ...
        try {
            ...
            let imageUrl = null;
            try {
                imageUrl = await generateAndStoreImage(title, finalUrl);
            } catch (imageError) {
                console.warn(`[LOG] Image generation failed for: ${title}`, imageError.message);
            }
            ...
            const { error: insertError } = await supabaseClient.from('latest_news').insert(newItem);
