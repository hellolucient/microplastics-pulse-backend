...
// Lazy initialization - only create client when needed and if credentials are available
let supabase = null;
...
function getSupabaseClient() {
  ...
  if (!supabase) {
    try {
      ...
      console.warn('[AI Usage Logger] Failed to initialize Supabase client:', error.message);
      return null;
    }
  }
...
const PRICING = {
  openai: {
    ...
    'dall-e-3': {
      standard: 0.040,  // $0.040 per image (1024x1024) — retired May 2026
      hd: 0.080         // $0.080 per image (1024x1024 HD)
    },
    'gpt-image-1': {
      low: 0.011,       // $0.011 per image (1024x1024)
      medium: 0.042,    // $0.042 per image (1024x1024)
      high: 0.167,      // $0.167 per image (1024x1024)
      auto: 0.042       // estimate at medium when auto
    }
  },
  anthropic: {
    'claude-3-sonnet': {
 ...
 */
function calculateImageCost(provider, model, quality = 'medium') {
  if (provider === 'openai') {
    ...
    if (pricing) {
      ...
      if (model.startsWith('gpt-image')) {
        return pricing[quality] ?? pricing.medium ?? 0.042;
      }
    }
  }
...
async function logAIUsage({
  ...
  try {
    ...
    if (!supabaseClient) {
      // If Supabase is not configured, just log to console and continue
      console.warn('[AI Usage Logger] Supabase not configured, skipping database log. Set SUPABASE_URL and SUPABASE_SERVICE_KEY to enable logging.');
      console.log(`[AI Usage] ${operationType} - ${provider}/${model} - $${costUsd.toFixed(6)} (not logged to DB)`);
      return;
    }
    ...
    const logData = {
      ...
      api_key_id: apiKeyId
      // Note: metadata column doesn't exist in the table, so we're not including it
    };
