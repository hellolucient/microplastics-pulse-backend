...
async function postSingleTweet(text, imageUrl) {
  try {
    // Get a user-context client for v2 API actions
    const userClient = client.v2.user;
    let mediaIds;
    ...
    if (imageUrl && imageUrl.startsWith('http')) {
      ...
      console.log('Uploading media to Twitter using API v2...');
      const contentType = imageResponse.headers.get('content-type') || 'image/jpeg';
      
      // The v2 upload method returns a single media ID string.
      const mediaId = await userClient.uploadMedia(buffer, {
    ...
    const tweet = await userClient.tweet({
      text,
