...
// Lazy initialization - only create client when needed and if tokens are available
let client = null;
...
function getTwitterClient() {
  ...
  if (!client) {
    try {
      ...
      console.warn('[Twitter] Failed to initialize Twitter client:', error.message);
      return null;
    }
  }
...
async function postSingleTweet(text, imageUrl) {
  try {
    ...
    if (imageUrl && imageUrl.startsWith('http')) {
      ...
      mediaId = await twitterClient.v1.uploadMedia(buffer, {
