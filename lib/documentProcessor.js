...
class DocumentProcessor {
  ...
  async processDocument(fileBuffer, filename, mimeType) {
    try {
      ...
      let finalFileType = this.getFileType(fileExtension);
      ...
      if (fileExtension === '.pdf' || mimeType === 'application/pdf') {
        ...
        convertedPdfBuffer = await this.convertWordToPDF(fileBuffer, filename);
        finalFileType = 'pdf';
      ...
      if (convertedPdfBuffer) {
        metadata.convertedFrom = fileExtension;
        metadata.convertedAt = new Date().toISOString();
        metadata.originalFileType = this.getFileType(fileExtension);
      }
      ...
      return {
        ...
        fileSize: convertedPdfBuffer ? convertedPdfBuffer.length : fileBuffer.length,
        wordCount: this.countWords(cleanedText),
        chunkCount: chunks.length,
        convertedPdfBuffer: convertedPdfBuffer // Include converted PDF buffer
      };
   ...
   */
  async convertWordToPDF(fileBuffer, filename) {
    try {
